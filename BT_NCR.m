function out = BT_NCR(seed,session,varargin)
%seed:随机数种子
rng(seed)
%% Parse parameters
%设置默认参数以及更新参数
...
%     filename = sprintf('BT_NCR_CiaoDVD_fold_%i_para.mat',i);
%     filename = sprintf('BT_NCR_MovieLens_fold_%i_para.mat',i);
%     filename = sprintf('BT_NCR_MovieLensSM_fold_%i_para.mat',i);
    filename = sprintf('BT_NCR_Tmall[buy.click]_fold_%i_para.mat',i);
%     filename = sprintf('BT_NCR_Tmall[buy.nobuy]_fold_%i_para.mat',i);
...
%---------- 保存结果---start-------
% resname = sprintf('BT_NCR_MovieLensSM_5fold_result.mat');
% resname = sprintf('BT_NCR_CiaoDVD_5fold_result.mat');
resname = sprintf('BT_NCR_Tmall[buy.click]_5fold_result.mat');
% resname = sprintf('BT_NCR_Tmall[buy.nobuy]_5fold_result.mat');
meanValue=mean(out);
allOut = out;
...
for i = 1:par.maxIter
    ...
    out = BT_NCR_RankEval(session,testIdx,U,V,theta,par); 
...
function r = lhFunc(sess,U,V,theta,par)
u = U(sess(1,1),:);
w = V(sess(:,2),:);
v = V(sess(:,3),:);
temp = zeros(size(w,1),par.F);
for i = 1:par.F
    w(:,[1,i]) = w(:,[i,1]);
    v(:,[1,i]) = v(:,[i,1]);
    temp(:,i) = w(:,1)./(w(:,1)+theta*v(:,1)).*prod(theta*w(:,2:end)./(v(:,2:end)+theta*w(:,2:end)),2);
end
p = ones(1,par.F);
for i = 1:size(temp,1)
    tp = p.*temp(i,:);
